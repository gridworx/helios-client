...
export function UserList({ organizationId, userType, onCountChange, searchQuery = '', statusFilter, platformFilter = 'all', advancedFilters = {}, columnConfigs, onStatusCountsChange, onDepartmentsChange, onNavigate }: UserListProps) {
  ...
  const fetchUsers = async () => {
    try {
      ...
      try {
        ...
        // Note: 'all' count should exclude deleted users since the 'All' tab doesn't show deleted
        const counts = {
          all: allUsersForCounts.filter((u: any) => u.userStatus !== 'deleted').length,
        ...
        // Extract unique departments for filter panel
        if (onDepartmentsChange) {
          ...
          onDepartmentsChange(uniqueDepts.sort());
        }
  ...
  // Filter columns based on columnConfigs (if provided)
  const columns = columnConfigs
    ? allColumns.filter(col => {
        // Always show checkbox and actions
        if (col.key === 'checkbox' || col.key === 'actions') return true;
        // Find matching config and check visibility
  ...
  const renderCell = (column: { key: string; className: string }, user: User, cellKey: string) => {
    const roleBadge = getRoleBadge(user.role);

    switch (column.key) {
      case 'checkbox':
        return (
          <div key={cellKey} className={column.className} onClick={(e) => e.stopPropagation()}>
            <input
              type="checkbox"
      ...
      case 'department':
        // Normalize department name - strip OU path prefixes like "/Staff/Sales" -> "Sales"
        const deptName = user.department
          ? (user.department.startsWith('/')
              ? user.department.split('/').filter(Boolean).pop() || user.department
              : user.department)
          : 'N/A';
        return <div key={cellKey} className={column.className}>{deptName}</div>;

      case 'company':
        return <div key={cellKey} className={column.className}>{user.company || 'N/A'}</div>;
   ...
   statusText = 'Active';
      ...
      case 'lastLogin':
        ...
        return (
          <div key={cellKey} className={`${column.className} ${loginStatusClass}`} title={lastLoginDate ? lastLoginDate.toLocaleString() : 'Never logged in'}>
            {loginText}
          </div>
        );
      ...
      case 'phone':
        return <div key={cellKey} className={column.className}>{user.workPhone || user.mobilePhone || 'N/A'}</div>;

      case 'title':
        return <div key={cellKey} className={column.className}>{user.jobTitle || 'N/A'}</div>;
      ...
      default:
        return <div key={cellKey} className={column.className}>-</div>;
    }
  };
  ...
  if (advancedFilters) {
    // Date Created filter
    if (advancedFilters.dateCreated) {
      const now = new Date();
      let cutoffDate: Date | null = null;

      switch (advancedFilters.dateCreated) {
        case 'recently':
    ...
    if (advancedFilters.lastLogin) {
      const now = new Date();
      filteredUsers = filteredUsers.filter(user => {
        switch (advancedFilters.lastLogin) {
          case 'never':
          ...
          case 'week':
            if (!user.lastLogin) return false;
            return new Date(user.lastLogin) >= new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
          case 'inactive':
            if (!user.lastLogin) return true;
            return new Date(user.lastLogin) < new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
          default:
            return true;
        }
      });
    ...
    if (advancedFilters.department) {
      filteredUsers = filteredUsers.filter(user =>
        user.department?.toLowerCase() === advancedFilters.department?.toLowerCase()
      );
    }
    ...
    if (advancedFilters.role) {
      filteredUsers = filteredUsers.filter(user =>
        user.role?.toLowerCase() === advancedFilters.role?.toLowerCase()
      );
    }

    // Integration status filter
    if (advancedFilters.integrationStatus) {
      filteredUsers = filteredUsers.filter(user => {
        const hasSyncedPlatform = user.googleWorkspaceId || user.microsoft365Id ||
          user.platforms.some(p => p === 'google_workspace' || p === 'microsoft_365');

        switch (advancedFilters.integrationStatus) {
          case 'synced':
            return hasSyncedPlatform;
          case 'local':
            return !hasSyncedPlatform;
          default:
            return true;
        ...
        {filteredUsers.length === 0 ? (
          ...
          <div className="table-body">
            {paginatedUsers.map(user => (
              ...
              >
                {columns.map((column) => renderCell(column, user, column.key))}
              </div>
            ))}
          </div>
        )}
